...
export const LinkedForumFlyout: React.FC<LinkedForumFlyoutProps> = ({
    ...
    const fetchForumsFromApi = useCallback(
        async (
            ...
            try {
                ...
                setHasMoreForums(items.length >= FORUM_PAGE_SIZE);

                if (pageNumber === 1) {
                    // Derive unique filter options only on fresh load
                    ...
                    setForumList(buildUniqueForumList(items, []));
                } else {
                    setForumList(prev => {
                        const existingIds = new Set(prev.map(f => f.id));
                        const newItems = buildUniqueForumList(items, prev).filter(
    ...
    const handleMultiSelectChange = useCallback(
        (key: ForumFilterKey) => {
            return (_option: OptionType, _checked: boolean, tree: object[]) => {
                ...
                pageRef.current = 1;
                fetchForumsFromApi(searchValue, nextFunc, nextSub, nextGeo, nextPeriod, 1);
            };
        },
        [
            fetchForumsFromApi,
    ...
    // Debounced re-fetch when search text changes (including clear)
    useEffect(() => {
    ...
    const handleLoadMoreForums = useCallback(() => {
        if (!hasMoreForums || isLoadingMoreForums || isLoading) return;
    ...
    }, [hasMoreForums, isLoadingMoreForums, isLoading, fetchForumsFromApi, searchValue, selectedFunctionFilter, selectedSubFunctionFilter, selectedGeoLevelFilter, selectedPeriodFilter]);

    const { scrollRef, handleScroll } = useInfiniteScroll({
        hasMoreItems: !showSelectedOnly && hasMoreForums,
        isLoadingMore: isLoadingMoreForums,
        onLoadMore: handleLoadMoreForums,
    });
        ...
        <Flex vertical className={styles['flyout-body']}>
            ...
            <div className={styles['content-wrapper']}>
                ...
                <div
                    ...
                    ref={showSelectedOnly ? undefined : scrollRef}
                    onScroll={showSelectedOnly ? undefined : handleScroll}
                >
                {isLoading ? (
                    ...
                    <>
                        ...
                        {!showSelectedOnly && isLoadingMoreForums && (
                            <div className={styles['loading-state']}>Loading more...</div>
                        )}
                    </>
                )}
                </div>
