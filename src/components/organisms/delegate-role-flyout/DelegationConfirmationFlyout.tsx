...
import DelegatedToolsTable from './DelegatedToolsTable';
...
import { editDelegation, saveDelegation } from '../../../services/delegation';
...
const DelegationConfirmationFlyout = ({
    open,
    roleIds,
    toolIds,
    selectedRoleDetails,
    ...
    const heading = 'Delegated Role Permissions';
    ...
    const { pageNumber, pageSize } = useSelector((s: RootState) => s.delegation);
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (isEditModeOn) {
                await editDelegation(payload);
            } else {
                await saveDelegation(payload);
            }
                ...
                primaryBtnProps={{
                    ...
                    onClick: async () => {
                        try {
                            setIsSaving(true);
                            await handleConfirm();
                            await dispatch(fetchDelegations({ pageNumber, pageSize })).unwrap?.();
...
export default DelegationConfirmationFlyout;
