...
const isHighlightSummaryToken = (token: string) =>
    token === HIGHLIGHT_SUMMARY_WIDGET_TOKEN ||
    token.startsWith(`${HIGHLIGHT_SUMMARY_WIDGET_TOKEN}@`);
...
const normalizeWidgetOrder = (widgetOrder?: string[]) => {
    ...
    let hasHighlightSummary = false;
    const seen = new Set<string>();
    const normalized: string[] = [];

    for (const token of widgetOrder) {
        if (isHighlightSummaryToken(token)) {
            if (hasHighlightSummary) {
                hasChanges = true;
                continue;
            }

            hasHighlightSummary = true;
            if (token !== HIGHLIGHT_SUMMARY_WIDGET_TOKEN) {
                hasChanges = true;
            }

            normalized.push(HIGHLIGHT_SUMMARY_WIDGET_TOKEN);
            seen.add(HIGHLIGHT_SUMMARY_WIDGET_TOKEN);
            continue;
        }
...
const normalizeTabWidgetsState = (tabState?: TabWidgetsState): TabWidgetsState => {
    ...
    if (normalizedWidgetOrder === tabState.widgetOrder) {
        return tabState;
    }
...
const performanceWidgetsByTabSlice = createSlice({
    ...
    reducers: {
        ...
        setHighlightSummaryState: (
            state,
            action: PayloadAction<{ tabId: number | string; summary: HighlightSummaryState }>,
        ) => {
            const k = keyOf(action.payload.tabId);
            state.byTab[k] = state.byTab[k] ?? {};
        ...
        removeFromWidgetOrder: (
            ...
            const prev = state.byTab[k].widgetOrder ?? [];
            const toRemove = new Set(action.payload.items);
            const removeHighlightSummary = action.payload.items.some(isHighlightSummaryToken);

            state.byTab[k].widgetOrder = normalizeWidgetOrder(
                prev.filter(
                    token =>
                        !toRemove.has(token) &&
                        !(removeHighlightSummary && isHighlightSummaryToken(token)),
                ),
            );
