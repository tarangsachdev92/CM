...
function EditNewForumScreen() {
        ...
        async (
            ...
            try {
                ...
                if (isForumOwnerContext) {
                    ...
                    setFilteredRoles(groupedResponse.map(item => ({ ...item, checked: false })));
                    if (groupedResponse.length > 0) {
                        setRolesAndUsers(prev => {
                            ...
                            const currentIds = new Set(groupedResponse.map(r => String(r.roleId)));
                            const preservedChecked = prev.filter(r => r.checked && !currentIds.has(String(r.roleId)));
                            const merged = groupedResponse.map(item => ({
                                ...item,
    ...
    const displayedRoleFlyoutItems = useMemo(() => {
        if (!isRoleOnlyMode) {
            // forumOwner context: use filteredRoles directly (unchanged behaviour)
            if (showSelectedOnly) return filteredRoles.filter(item => item.checked);
            return filteredRoles;
        }
        // role flyout: filter by current search results to prevent mixing preserved roles
        if (showSelectedOnly) return rolesAndUsers.filter(item => item.checked);
        if (searchKeyword.trim() !== '' && !isRoleDataLoading) {
            const currentIds = new Set(filteredRoles.map(r => String(r.roleId)));
            return rolesAndUsers.filter(item => currentIds.has(String(item.roleId)));
        }
        return rolesAndUsers;
    }, [isRoleOnlyMode, rolesAndUsers, filteredRoles, showSelectedOnly, searchKeyword, isRoleDataLoading]);
