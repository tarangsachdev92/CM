...
import { getForumDetails } from '../../../store/thunks/getForumDetails';
...
function ForumsTable({ onEditForum, refreshKey, triggerToast }: ForumTableProps) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            await dispatch(getForumDetails(request));
        } finally {
            setLoading(false);
        }
    }, [
        ...
        filters.forEach(filter => {
            filter.selectedFilters.forEach(({ label, value }) => {
                ...
                appliedFilterArray.push({
                    columnName: isStatus ? label : filter.id,
