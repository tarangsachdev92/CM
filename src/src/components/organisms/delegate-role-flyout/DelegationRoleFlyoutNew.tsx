...
import DelegationConfirmationFlyoutNew from './DelegationConfirmationFlyoutNew';
import { Tooltip } from 'antd';
import { getDelegationDetailsByIdNew, getRolesByUserForDelegationNew, getToolsByRoleIdForDelegationNew, getUserForDelegationNew } from '../../../services/delegationNew';

function DelegationRoleFlyoutNew({
    isOpen,
    isEditModeOn,
    onCancelClick,
    selectedRowId,
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            const usersData = await getUserForDelegationNew();
    ...
    useEffect(() => {
        if (selectedRowId !== '0' && selectedRowId !== '') {
            (async () => {
                const response = await getDelegationDetailsByIdNew({
    ...
    const getRolesByUserSelected = async (selectedUser: any) => {
        const roleList = await getRolesByUserForDelegationNew({
    ...
    const getToolsByRolesSelected = async (selectedRoles: string) => {
        const toolsList = await getToolsByRoleIdForDelegationNew({ roleId: selectedRoles });
    ...
    return (
        <div className={styles['outer-container']}>
            ...
            <DelegationConfirmationFlyoutNew
                open={isConfirmOpen}
...
export default DelegationRoleFlyoutNew;
