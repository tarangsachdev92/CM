...
function KgResultsNode({ node }: NodeRendererProps) {
  ...
  const columnsData = columns.map((c) => ({ width: '200px', key: c.key, dataIndex: c.key, title: c.label || c.key, render: (value: unknown) => <div>{String(value)}</div> }));
  const rows = (node.props.rows as RowDef[]) || [];
...
function SuggestionsNode({ node, onAction }: NodeRendererProps) {
    ...
    <Stack direction="row" flexWrap="wrap" gap={1} sx={{ mt: 1 }}>
      {items.map((text, i) => (
        <Chip
          ...
          size="medium"
          clickable
          onClick={() => onAction?.({ type: "suggestion", text })}
